...
from inpaint.propainter.model.modules.flow_comp_raft import RAFT_bi
from inpaint.propainter.model.recurrent_flow_completion import RecurrentFlowCompleteNet
from inpaint.propainter.model.propainter import InpaintGenerator
from inpaint.propainter.core.utils import to_tensors
from inpaint.propainter.model.misc import get_device
...
class ProPainter:
    def __init__(self, video_path,video_out_path, mask_path=None, callback=None) -> None:
        ...
        print("video_out_path : ",self.video_out_path)
    ...
    def __call__(self):
        ...
        frames, fps, size, video_name = self.read_frame_from_videos(self.video_path)
        ...
        if args.mode == 'video_inpainting':
            ...
            flow_masks, masks_dilated = self.read_mask(self.mask_path, frames_len, size, 
            ...
            frames, flow_masks, masks_dilated, size = self.extrapolation(frames, (args.scale_h, args.scale_w))
            w, h = size
        else:
        ...
        ##############################################
        fix_raft = RAFT_bi(config.RAFT_MODEL_PATH, device)
        
        fix_flow_complete = RecurrentFlowCompleteNet(config.FLOW_MODEL_PATH)
        ...
        for f in tqdm(range(0, video_length, neighbor_stride)):
            ...
            ref_ids = self.get_ref_index(f, neighbor_ids, video_length, args.ref_stride, ref_num)
        ...
        if args.save_frames:
            for idx in range(video_length):
                ...
                img_save_root = os.path.join(self.video_out_path, 'frames', str(idx).zfill(4)+'.png')
        ...
        imageio.mimwrite(self.video_out_path, masked_frame_for_save, fps=fps, quality=7)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('-i', '--video', required=True, help='输入视频文件路径')
    parser.add_argument('-m', '--mask', required=True, help='掩码图片路径')
    args = parser.parse_args()
    #console运行，这里代码需要修复
    inpainter = ProPainter(video_path, mask_path, clip_gap=config.STTN_MAX_LOAD_NUM)
    inpainter()